//
//  OutDoorDutyTimeLogViewController.swift
//  WrkplanPayroll
//
...
class OutDoorDutyTimeLogViewController: UIViewController , UITableViewDelegate, UITableViewDataSource {
...
//        cell.label_date.text = dateFormatterPrint.string(from: date!)
    ...
    func loadData(){
        ...
        let resultString = inputFormatter.string(from: showDate!)
        //---code to format date, ends---
        let url = "\(BASE_URL)od/log/detail/\(swiftyJsonvar1["company"]["corporate_id"].stringValue)/\(OutdoorDutyLogListViewController.od_request_id!)/\(resultString)/"
